...
\fancyfoot[C]{\thepage}
\lstset{basicstyle=\small\ttfamily,breaklines=true,frame=single,columns=fullflexible,keepspaces=true,showstringspaces=false,backgroundcolor=\color{gray!6}}
...
\vspace{2.0cm}
\begin{tabular}{rl}
\textbf{课程名称：} & 信息安全技术\\[0.35em]
\textbf{考查方式：} & 课程报告与代码复现\\[0.35em]
...
我们选择 MBRS 和 DWSF 作为复现对象，主要有三点原因。

第一，两篇论文都属于信息安全与数字水印方向，均发表在 ACM MM，满足课程对 CCF-B 以上论文的要求。第二，两者方法具有明显继承关系和对比价值：MBRS 代表较典型的固定尺寸端到端深度鲁棒水印框架，重点解决 JPEG 压缩；DWSF 则进一步面向真实应用中的任意分辨率和几何扰动，尝试把局部水印、同步定位和消息融合结合起来。第三，两篇论文均公开了代码，有利于进行实际复现和实验对比。
...
\caption{图 1 DWSF 官方框架图。 DWSF 将任意分辨率图像划分为多个局部候选块，在若干图像块中分散嵌入相同消息；提取阶段先通过同步模块定位水印区域，再由解码器和融合策略恢复最终消息。}
\end{figure}
...
\caption{图 2 模型结构对比。 MBRS 采用固定尺寸图像块上的编码器、噪声层、解码器流程，核心创新在真实与模拟 JPEG 的混合 mini-batch 训练；DWSF 在编码器-解码器之外加入块选择、分散嵌入、同步定位和消息融合，更强调实际图像场景。}
\end{figure}
...
\begin{center}
\begin{tabular}{l}
\texttt{pip install torch torchvision kornia numpy Pillow scipy tqdm opencv-python pyyaml crc8} \\
\texttt{python code/patch\_mps.py} \\
\texttt{python code/train.py --model mbrs --epochs 30} \\
\texttt{python code/train.py --model dwsf --epochs 50} \\
\texttt{python code/experiments.py} \\
\texttt{python code/visualize.py}
\end{tabular}
\end{center}

其中，\texttt{code/train.py} 负责保存模型权重和训练历史；\texttt{code/experiments.py} 负责输出 \texttt{results/experiment\_data.json} 和 \texttt{results/images/} 中的水印样例；\texttt{code/visualize.py} 负责生成 \texttt{results/charts/} 下的训练曲线、鲁棒性柱状图、雷达图、结构图和综合对比表。报告中的图 1 至图 12 均来自仓库中的论文代码资产或本次实验产物，因此报告、代码和结果之间是一一对应的。
...
\caption{图 3 PSNR 收敛曲线。 MBRS 的 PSNR 上升更快，最终达到约 34.6 dB；DWSF 逐步提升至约 28.4 dB。说明 MBRS 在当前复现设置下具有更好的水印图像质量。}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{../results/charts/A2_ber_curve.png}
\caption{图 4 BER 收敛曲线。 两个模型的 BER 都随训练下降。MBRS 在较早阶段已接近 0；DWSF 也在训练推进过程中逐渐收敛，说明其编码器-解码器在轻量训练后能够建立稳定消息通道。}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{../results/charts/A3_loss_curve.png}
\caption{图 5 训练损失曲线。 Encoder loss 和 Decoder loss 均呈下降趋势，说明模型在图像重建和消息恢复两个目标上都完成了有效优化。}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{../results/charts/C1_watermark_samples.png}
\caption{图 6 水印样例可视化。 每组三列分别为原图、水印图和 10 倍放大的残差图。前三行对应 MBRS，后三行对应 DWSF。可以看到水印图与原图在整体视觉上仍保持一致，但残差图显示出编码器在局部像素中嵌入了可学习扰动。}
\end{figure}
...
\centering
\small
\begin{tabular}{p{0.22\textwidth}p{0.18\textwidth}p{0.18\textwidth}p{0.18\textwidth}p{0.18\textwidth}}
\toprule
...
\caption{图 7 不同攻击下的 BER 对比。 红色虚线表示 BER=5\% 的参考阈值。两种模型均在 6/12 种攻击下达到 BER<5\%，在强压缩、强模糊和裁剪等扰动较强的条件下，BER 变化也清晰展示了不同攻击对水印信道的影响。}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{../results/charts/B2_psnr_bar.png}
\caption{图 8 不同攻击下的 PSNR 对比。 MBRS 在大部分条件下 PSNR 高于 DWSF，说明其水印图像质量更好；裁剪和亮度变化会显著降低 PSNR。}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{../results/charts/B3_radar.png}
\caption{图 9 鲁棒性雷达图。 雷达图以 1-BER 作为鲁棒性得分，展示两种模型在典型攻击下的综合表现。两者在 clean 和噪声条件下得分较高，在裁剪和强压缩下得分较低。}
\end{figure}
...
在高斯噪声攻击下，两种模型都表现出较强鲁棒性。即使噪声标准差达到 σ=0.10，MBRS 的 BER 仍为 0.0006，DWSF 的 BER 为 0.0024。这说明编码器嵌入的水印信号并不是集中在少数脆弱像素上，而是具有一定冗余性，能够抵抗随机噪声扰动。高斯噪声不会系统性改变图像结构，只是在局部像素上叠加随机变化，因此深度解码器仍能从整体统计模式中恢复消息。
...
\caption{图 10 攻击效果展示。 图中展示了水印图像在噪声、模糊、压缩、裁剪和亮度变化下的视觉变化。部分攻击从视觉上看不一定严重，但对 BER 可能造成明显影响，说明水印鲁棒性不能只依赖肉眼观察，需要用消息恢复指标评价。}
\end{figure}
...
\caption{图 11 参数量与性能权衡。 MBRS 参数量更大，Clean PSNR 更高；DWSF 参数量更小，Clean BER 同样为 0，体现出更好的轻量化潜力。}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{../results/charts/D2_summary_table.png}
\caption{图 12 综合对比表。 MBRS 在图像质量、轻 JPEG 压缩和亮度降低条件下更优；DWSF 在参数量和部分亮度变化条件下具有优势，并具备面向任意分辨率图像的系统设计潜力。}
\end{figure}
...
运行 \texttt{python -c "import torch; print(torch.\_\_version\_\_); print(torch.backends.mps.is\_available())"}，展示 PyTorch 版本与 MPS 可用性。

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{screenshots/screenshot_5_1_env_check.png}
\caption{图13 环境与依赖检查。 确认 PyTorch 2.12.1 安装正常，MPS 后端可用；MPS 适配补丁成功应用。}
\end{figure}

\subsection{MBRS 训练过程截图}

运行 \texttt{python code/train.py --model mbrs --epochs 30}，展示 epoch、loss、PSNR、SSIM、BER 等训练输出。

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{screenshots/screenshot_5_2_mbrs_train.png}
\caption{图14 MBRS 训练过程。 30 个 epoch 内 PSNR 从 18.0 dB 提升至 34.6 dB，BER 降至 0，训练耗时约 27.5 分钟。}
\end{figure}

\subsection{DWSF 训练过程截图}

运行 \texttt{python code/train.py --model dwsf --epochs 50}，展示 DWSF 的训练收敛过程。

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{screenshots/screenshot_5_3_dwsf_train.png}
\caption{图15 DWSF 训练过程。 50 个 epoch 内 PSNR 从 9.1 dB 提升至 28.4 dB，BER 降至 0.0013，训练耗时约 65 分钟。}
\end{figure}

\subsection{鲁棒性测试截图}

运行 \texttt{python code/experiments.py}，展示 Clean、JPEG、Blur、Noise、Crop、Brightness 等攻击下的 PSNR 和 BER 输出。

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{screenshots/screenshot_5_4_robustness.png}
\caption{图16 鲁棒性测试输出。 12 种攻击条件下两个模型的 PSNR 和 BER 完整结果。}
\end{figure}

\subsection{结果文件截图}

展示 \texttt{results/charts} 和 \texttt{results/images} 目录，说明实验生成了图表和水印样例。

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{screenshots/screenshot_5_5_results.png}
\caption{图17 生成的实验结果文件。 包含 11 张图表、18 张水印样本和实验数据 JSON。}
\end{figure}
...
补充实验可以设置三组对比：原始 MBRS、原始 DWSF、组合改进方案。攻击类型重点选择 JPEG Q=50、裁剪 10\%、高斯噪声 σ=0.10 和亮度变化，指标仍使用 PSNR、SSIM 和 BER。预期上，组合方案可能在强压缩和局部破坏场景下获得更稳定的消息恢复能力，同时保留深度水印的不可感知性。

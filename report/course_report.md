...
## 基于深度学习的鲁棒图像水印方法复现与对比：MBRS 与 DWSF

**课程名称：** 信息安全技术  
**考查方式：** 课程报告与代码复现  
**选题方向：** 方向 2：复现两篇同领域 CCF-B 以上论文并对比优缺点  
**研究领域：** 数字水印、深度学习图像水印、鲁棒盲水印  
**复现论文：**
...
![DWSF 官方框架图](../code/DWSF/IMG/framework.png)

**图 1 DWSF 官方框架图。** DWSF 将任意分辨率图像划分为多个局部候选块，在若干图像块中分散嵌入相同消息；提取阶段先通过同步模块定位水印区域，再由解码器和融合策略恢复最终消息。
...
![模型结构对比](../results/charts/C3_architecture.png)

**图 2 模型结构对比。** MBRS 采用固定尺寸图像块上的编码器、噪声层、解码器流程，核心创新在真实与模拟 JPEG 的混合 mini-batch 训练；DWSF 在编码器-解码器之外加入块选择、分散嵌入、同步定位和消息融合，更强调实际图像场景。
...
其中，`code/MBRS` 和 `code/DWSF` 保存两篇论文的核心模型代码；`code/train.py` 是我们整理后的轻量化训练脚本；`code/experiments.py` 用于生成水印样例并进行鲁棒性测试；`code/visualize.py` 用于根据实验数据生成图表；`code/patch_mps.py` 用于适配 Apple Silicon Mac 的 MPS 后端，主要处理设备选择、`.cuda()` 调用和 `pin_memory` 等问题。
...
1. 将设备选择从固定 CUDA 改为优先使用 MPS，其次 CUDA，最后 CPU。
2. 移除或替换部分 `.cuda()` 调用，统一使用 `.to(device)`。
3. 在 DataLoader 中去除对当前环境不必要的 `pin_memory=True`。
4. 移除不适合单 MPS 设备的 `DataParallel`。
5. 对 DWSF 中部分 CUDA seed 设置增加兼容处理。
6. 编写统一训练与评估脚本，避免分别手动修改两套代码造成结果口径不一致。
...
# 2. Apple Silicon Mac 环境执行 MPS 适配
...
其中，`code/train.py` 负责保存模型权重和训练历史；`code/experiments.py` 负责输出 `results/experiment_data.json` 和 `results/images/` 中的水印样例；`code/visualize.py` 负责生成 `results/charts/` 下的训练曲线、鲁棒性柱状图、雷达图、结构图和综合对比表。报告中的图 1 至图 12 均来自仓库中的论文代码资产或本次实验产物，因此报告、代码和结果之间是一一对应的。

### 3.6 评价指标

本文使用以下指标评价模型：

**PSNR。** 峰值信噪比用于衡量水印图像与原图之间的像素级差异，单位为 dB。PSNR 越高，说明图像失真越小，不可感知性越好。在图像水印任务中，PSNR 通常用于评价嵌入水印后图像质量。

**SSIM。** 结构相似性用于衡量图像在亮度、对比度和结构上的相似程度。相比 PSNR，SSIM 更接近人眼感知质量。由于当前 `experiment_data.json` 中部分 SSIM 字段记录异常，本报告主要采用图表和整理结果中的 Clean SSIM 值进行讨论，对攻击条件下的分析重点放在 PSNR 和 BER 上。

**BER。** Bit Error Rate 表示解码消息与原始消息之间不一致的比特比例，是衡量水印提取准确性的关键指标。BER 越低，说明消息恢复越准确。BER 为 0 表示所有比特均正确恢复；BER 接近 0.5 表示解码效果接近随机猜测。
...
![PSNR 收敛曲线](../results/charts/A1_psnr_curve.png)

**图 3 PSNR 收敛曲线。** MBRS 的 PSNR 上升更快，最终达到约 34.6 dB；DWSF 逐步提升至约 28.4 dB。说明 MBRS 在当前复现设置下具有更好的水印图像质量。

![BER 收敛曲线](../results/charts/A2_ber_curve.png)

**图 4 BER 收敛曲线。** 两个模型的 BER 都随训练下降。MBRS 在较早阶段已接近 0；DWSF 也在训练推进过程中逐渐收敛，说明其编码器-解码器在轻量训练后能够建立稳定消息通道。

![训练损失曲线](../results/charts/A3_loss_curve.png)

**图 5 训练损失曲线。** Encoder loss 和 Decoder loss 均呈下降趋势，说明模型在图像重建和消息恢复两个目标上都完成了有效优化。
...
![水印样例可视化](../results/charts/C1_watermark_samples.png)

**图 6 水印样例可视化。** 每组三列分别为原图、水印图和 10 倍放大的残差图。前三行对应 MBRS，后三行对应 DWSF。可以看到水印图与原图在整体视觉上仍保持一致，但残差图显示出编码器在局部像素中嵌入了可学习扰动。
...
![BER 攻击对比](../results/charts/B1_ber_bar.png)

**图 7 不同攻击下的 BER 对比。** 红色虚线表示 BER=5% 的参考阈值。两种模型均在 6/12 种攻击下达到 BER<5%，在强压缩、强模糊和裁剪等扰动较强的条件下，BER 变化也清晰展示了不同攻击对水印信道的影响。

![PSNR 攻击对比](../results/charts/B2_psnr_bar.png)

**图 8 不同攻击下的 PSNR 对比。** MBRS 在大部分条件下 PSNR 高于 DWSF，说明其水印图像质量更好；裁剪和亮度变化会显著降低 PSNR。

![鲁棒性雷达图](../results/charts/B3_radar.png)

**图 9 鲁棒性雷达图。** 雷达图以 1-BER 作为鲁棒性得分，展示两种模型在典型攻击下的综合表现。两者在 clean 和噪声条件下得分较高，在裁剪和强压缩下得分较低。
...
![攻击效果展示](../results/charts/C2_attack_effects.png)

**图 10 攻击效果展示。** 图中展示了水印图像在噪声、模糊、压缩、裁剪和亮度变化下的视觉变化。部分攻击从视觉上看不一定严重，但对 BER 可能造成明显影响，说明水印鲁棒性不能只依赖肉眼观察，需要用消息恢复指标评价。
...
![参数与性能权衡](../results/charts/D1_param_perf.png)

**图 11 参数量与性能权衡。** MBRS 参数量更大，Clean PSNR 更高；DWSF 参数量更小，Clean BER 同样为 0，体现出更好的轻量化潜力。

![综合对比表](../results/charts/D2_summary_table.png)

**图 12 综合对比表。** MBRS 在图像质量、轻 JPEG 压缩和亮度降低条件下更优；DWSF 在参数量和部分亮度变化条件下具有优势，并具备面向任意分辨率图像的系统设计潜力。
...
运行 `python -c "import torch; print(torch.__version__); print(torch.backends.mps.is_available())"`，展示 PyTorch 版本与 MPS 可用性。

![环境检查](screenshots/screenshot_5_1_env_check.png)

**图13 环境与依赖检查。** 确认 PyTorch 2.12.1 安装正常，MPS 后端可用；MPS 适配补丁成功应用。

### 5.2 MBRS 训练过程截图

运行 `python code/train.py --model mbrs --epochs 30`，展示 epoch、loss、PSNR、SSIM、BER 等训练输出。

![MBRS训练](screenshots/screenshot_5_2_mbrs_train.png)

**图14 MBRS 训练过程。** 30 个 epoch 内 PSNR 从 18.0 dB 提升至 34.6 dB，BER 降至 0，训练耗时约 27.5 分钟。

### 5.3 DWSF 训练过程截图

运行 `python code/train.py --model dwsf --epochs 50`，展示 DWSF 的训练收敛过程。

![DWSF训练](screenshots/screenshot_5_3_dwsf_train.png)

**图15 DWSF 训练过程。** 50 个 epoch 内 PSNR 从 9.1 dB 提升至 28.4 dB，BER 降至 0.0013，训练耗时约 65 分钟。

### 5.4 鲁棒性测试截图

运行 `python code/experiments.py`，展示 Clean、JPEG、Blur、Noise、Crop、Brightness 等攻击下的 PSNR 和 BER 输出。

![鲁棒性测试](screenshots/screenshot_5_4_robustness.png)

**图16 鲁棒性测试输出。** 12 种攻击条件下两个模型的 PSNR 和 BER 完整结果。

### 5.5 结果文件截图

展示 `results/charts` 和 `results/images` 目录，说明实验生成了图表和水印样例。

![结果文件](screenshots/screenshot_5_5_results.png)

**图17 生成的实验结果文件。** 包含 11 张图表、18 张水印样本和实验数据 JSON。
